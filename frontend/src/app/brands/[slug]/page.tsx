...
export async function generateMetadata({ params }: BrandDetailPageProps): Promise<Metadata> {
  ...
  try {
    ...
    return {
      title: `${brand.name} 생일 혜택 - Zup`,
      description: `${brand.name} 생일 혜택의 앱 필요 여부, 멤버십 조건, 사용 기간, 공식 출처를 확인하세요.`,
    };
  } catch {
    return {
      title: "브랜드 생일 혜택 - Zup",
    ...
    <div className="space-y-10">
      <section className="rounded-xl border border-border bg-white p-6">
        ...
        <div className="mt-5 flex flex-wrap gap-2 text-sm">
          ...
          {brand.officialUrl ? (
            <a className="rounded-lg border border-border px-3 py-2 font-semibold" href={brand.officialUrl} target="_blank" rel="noreferrer">
              공식 홈페이지
            </a>
          ) : null}
          {brand.membershipUrl ? (
            <a className="rounded-lg border border-border px-3 py-2 font-semibold" href={brand.membershipUrl} target="_blank" rel="noreferrer">
              멤버십 안내
            </a>
          ) : null}
          {brand.appUrl ? (
            <a className="rounded-lg border border-border px-3 py-2 font-semibold" href={brand.appUrl} target="_blank" rel="noreferrer">
              앱 안내
            </a>
          ) : null}
        </div>
      ...
      <section>
        <SectionHeader title={`${brand.name} 공개 혜택`} description="공식 출처 검수 후 공개된 생일 혜택만 표시됩니다." />
