...
- 공개 전 관리자는 Benefit 상세에서 요약, 이용 조건, 공식 출처를 검수하고 공식 출처가 확인된 경우에만 `PUBLISHED`로 전환한다.
...
`PUBLISHED` 전환 전 확인 항목:

- 요약이 사용자에게 혜택 내용을 충분히 설명하는지 확인
- 혜택 상세 리스트를 정리하고, 브랜드명이 이미지 로고로만 제공된 항목은 자동 확정하지 않고 수동 검수
- 이용 조건, 구매 조건, 사용 가능 기간을 정리
- 공식 출처가 최소 1개 이상 활성 상태인지 확인
- 최근 확인일을 입력
- 공식 출처가 없거나 조건이 불명확하면 `PUBLISHED`로 전환하지 않음

`BenefitDetailItem`은 공개 혜택 카드의 대표 혜택 목록으로 사용된다. Public 화면에는 `PUBLISHED + isActive=true` Benefit의 active detail item만 표시한다.
